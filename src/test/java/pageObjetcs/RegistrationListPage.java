...
//			jswait.loadClick("//iron-list[@id='list']", "//span[contains(.,'selenium_blackout_always')]");
	...
	public void createRewardwithparameters(String reward) throws Exception {
		CommonObjects commonObjects = new CommonObjects();
		RewardTypePage rewardTypePage = new RewardTypePage();
		jswait.loadClick(rewards);
		jswait.loadClick(createNewReward);
		jswait.loadSendKeys(rewardNameInput, reward);
		jswait.loadSendKeys(rewardDescription, reward + " reward by selenium");
		jswait.loadSendKeys(flowClassInput, "com.flytxt.fulfillment.flows.sample.SampleFlow");
		jswait.loadClick(selectSampleFlow);
		jswait.loadClick(saveReawrdbtn);
		commonObjects.filterName(reward);
		rewardTypePage.expandReward(reward);
		rewardTypePage.createRewardParameters(reward + " parameter", reward + " parameter_Value", "TEXT", "1");

...
#@NX-createDKJob@initBrowser @closeBrowser
...
@NX-createDiscoveredClustersList @initBrowser @closeBrowser 
...
@NX-createCustomerDrivenEvents @initBrowser @closeBrowser 
...
@NX-3482 @NX-createDndList @initBrowser @closeBrowser 
Scenario: Calculate Targets - Creating DNC Exclusion list. 3482 
	Given login 
	Then navigate to data foundation 
	Then navigate to registration list 
...
@NX-3483 @NX-createMandatoryExclusionList @initBrowser @closeBrowser 
...
@NX-8842 @NX-createProfileField @initBrowser @closeBrowser
Scenario: generate randrom registration list and upload 
	Given login 
	Then navigate to data foundation 
	Then check and add profile fields 
	
@NX-createDefaultReward @initBrowser  
...
@precondition-Predefinied_targets @initBrowser  @closeBrowser
...
@SavedsegmentsEdit @initBrowser  @closeBrowser
Scenario: create saved segment as precondition 
	Given login 
	Then navigate to landing page 
	Then navigate to precision marketer 
...
@NDX-CreateNewTrackSource @initBrowser  @closeBrowser
Scenario Outline: create the track source 
	Given login 
	Then navigate to data foundation 
	Then navigate to fulfillment configuration 
	Then navigate to tracking data sources 
	Then create new tracking source with name "<Trackname>" 
	Examples: 
		|Trackname|
		|A_track_Sel|
		|B_track_Sel|
...
@NDX-CreateFrequency @initBrowser  @closeBrowser
...
@NDX-CreateBlackout @initBrowser  @closeBrowser
...
@NDX-CreateInventory @initBrowser @closeBrowser
Scenario Outline: create inventory 
Given login 
Then navigate to configuration management 
# Then create new inventory "<inventory>"
Then create new inventory with "<inventory>","<frequency>","<blackout>" 
Examples: 
...
@NDX-CreateReward @initBrowser @closeBrowser
...
@ProfileUploadThroughDK @NDX-10317 @initBrowser @closeBrowser
...
@MetricEventUploadThroughDK @NDX-10316 @NDX-10318 @initBrowser @closeBrowser
...
Then provide file in location "/usr/local/flytxt/seleniumTrigger" for trigger with csv file "selenium_list_metric_event.csv" 

#  make sure that selenium_list_date.csv have the nearest date

@DateUploadThroughDK @initBrowser @closeBrowser
...
# UCG Age profile Upload Selenium_UCGAgeProfile.csv

@UCGAgeUploadThroughDK @initBrowser @closeBrowser
Scenario: Upload UCG Age Profile field through dk 
Given login 
Then create a dk job "selUcgAgeProfile" to upload the date fields 
Then navigate to landing page 
Then navigate to data foundation 
Then click Data Fusion Engine 
Then filter the job "selUcgAgeProfile" and activate it 
Then provide file in location "/usr/local/flytxt/seleniumProfile" for trigger with csv file "Selenium_UCGAgeProfile.csv" 



#//////   real time rte for falcon tp ////
	
@NDX-realTimeRTE @initBrowser @closeBrowser
...
@NDX-realTimeReacharge @initBrowser @closeBrowser
Scenario: create a streaming attribute with real time rte 
	Given login 
	Then navigate to data foundation 
	Then navigate to streaming attributes 
